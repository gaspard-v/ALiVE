...
export default class Rooms extends Phaser.Scene {
    ...
    create() {
        ...
        const roomBackground = this.add.sprite(
            width / 2,
            height / 2,
            this.roomFile
        );
        ...
        this.doors.map((doorData) => {
            getNameDestinationRoom(doorData['destination_room_uuid']).then(
    ...
    chargeRoom(key) {
        ...
        if (isReflectionDelayOver.bool == true) {
            if (!this.scene.isActive('reflectionButton')) {
                const reflectionButton = new ReflectionButton(
                    'reflectionButton'
                );
    ...
    bringPrompt(objectData) {
        ...
        if (this.scene.isActive(key) === null) {
            const promptObject = new PromptObject(
                key,
                this.scene.key,
                objectData
            );
