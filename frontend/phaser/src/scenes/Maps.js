...
export default class Maps extends Phaser.Scene {
    ...
    preload() {
        this.load.image(
            'mapBackground',
            '/static/assets/images/map/mapBackground.png'
        );
        ...
        this.load.image(
            'visitButton',
            '/static/assets/images/map/visitButton.png'
        );
        this.load.image(
            'quitButton',
            '/static/assets/images/map/quitButton.png'
        );
        this.load.image(
            'backButton',
            '/static/assets/images/map/backButton.png'
        );
        this.load.image(
            'promptBackground',
            '/static/assets/images/menu/promptBackground.png'
        );
        this.load.image(
            'closeIconWhite',
            '/static/assets/images/menu/closeIconWhite.png'
        );
    }
        ...
        const accessiblePlaces = this.map.map((place) => {
            ...
            this.add
                .sprite(
                    width / 2,
                    height / 2,
                    this.textures.get(this.map[0]['mapFile'])
                )
                .setScale(1.3);
            ...
            axios
                .get(`${API_HOST}/api/map/${this.map[0]['map_uuid']}`, {
                    params: {
                        full: true,
                    },
                })
    ...
    chargeRoomBackground = async (response) => {
        ...
        return await axios
            .get(`${API_HOST}/api/file/room/${uuidRoomBackground}`)
            .then(async (responseFetch) => {
                const roomBackgroundKey =
                    'image_' + responseFetch.data.message[0]['uuid'];
                response['roomFile'] = roomBackgroundKey;
                if (!this.textures.exists(roomBackgroundKey)) {
                    this.textures.addBase64(
                        roomBackgroundKey,
                        responseFetch.data.message[0]['data']
                    );
                }
                await this.chargeRoomObjects(response['objects']).then(
                    (res) => (response['objects'] = res)
                );
            })
            .catch((e) => console.log(e));
    };
    ...
    chargeRoomObjects = async (response) => {
        ...
        for (let object of response) {
            ...
            await axios
                .get(`${API_HOST}/api/file/object/${uuidObject}`)
                .then((responseFetch) => {
