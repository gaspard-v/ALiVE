...
export default class Reflection extends Phaser.Scene {
    ...
    create() {
        ...
        const maskGraphics = this.make
            .graphics()
            .fillStyle(0xffffff, 1)
            .fillRoundedRect(
                xRectTopLeft,
                yRectTopLeft,
                inventoryBackground.width,
                inventoryBackground.height,
                borderRadius
            );
        ...
        const inventoryMaskGraphics = this.make
            .graphics()
            .fillStyle(0xffffff, 1)
            .fillRoundedRect(
                xInventoryRectTopLeft,
                yInventoryRectTopLeft,
                spriteInventory.width,
                spriteInventory.height,
                borderRadius
            );
        ...
        this.itemsInventory = new ItemContainer(
            x + 960,
            y,
            objectData,
            this,
    ...
    addToSelection(object) {
        for (let i = 0; i < this.items.objectData.length; i++) {
            if (!this.items.objectData[i].object) {
                ...
                const frame = this.items.container.getByName(
                    'frame_' + object.index
                );
